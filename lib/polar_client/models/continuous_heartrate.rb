...
module PolarClient
  ...
  class ContinuousHeartrate
    # Absolute link to user owning the continuous heart rate samples
    attr_accessor :polar_user

    # Result date of the Continuous Heart Rate
    attr_accessor :date
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `PolarClient::ContinuousHeartrate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `PolarClient::ContinuousHeartrate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [polar_user, date, heart_rate_samples].hash
    end
