  ...
  class Exercise
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `PolarClient::Exercise` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `PolarClient::Exercise`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, upload_time, polar_user, transaction_id, device, device_id, start_time, start_time_utc_offset, duration, calories, distance, heart_rate, training_load, sport, has_route, club_id, club_name, detailed_sport_info, fat_percentage, carbohydrate_percentage, protein_percentage, running_index, training_load_pro].hash
    end
