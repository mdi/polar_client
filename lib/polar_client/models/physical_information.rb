  ...
  class PhysicalInformation
    ...
    def initialize(attributes = {})
      if !attributes.is_a?(Hash)
        fail ArgumentError, "The input argument (attributes) must be a hash in `PolarClient::PhysicalInformation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if !self.class.attribute_map.key?(k.to_sym)
          fail ArgumentError, "`#{k}` is not a valid attribute in `PolarClient::PhysicalInformation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def ==(other)
      ...
      self.class == other.class &&
        ...
        resting_heart_rate == other.resting_heart_rate &&
        aerobic_threshold == other.aerobic_threshold &&
        anaerobic_threshold == other.anaerobic_threshold &&
        vo2_max == other.vo2_max &&
        weight_source == other.weight_source
    end
